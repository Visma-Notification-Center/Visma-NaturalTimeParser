...
namespace Pathoschild.NaturalTimeParser.Tests.Extensions
{
	...
	public class DotLiquidTests
	{
		...
		*********/
		...
		[TestCase("{{ date | date:'yyyy-MM-dd' }}", ExpectedResult = "2000-01-01")]
		[TestCase("{{ date | date_offset:'10 years 2 months 3 days' |date:'yyyy-MM-dd' }}", ExpectedResult = "2010-03-04")]
		[TestCase("{{ date | date_offset:'-1 year' | date:'yyyy' }}", ExpectedResult = "1999")]
		[TestCase("{{ date | date_offset:'1 day ago' | date:'yyyy-MM-dd' }}", ExpectedResult = "1999-12-31")]
		[TestCase("{{ date | date_offset:'14 days' | date:'yyyy-MM-dd' }}", ExpectedResult = "2000-01-15")]
